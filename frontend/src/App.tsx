...
function TextLayers({scene,onChange}:{scene:Scene;onChange:(layers:NonNullable<Scene["font_json"]["layers"]>)=>void}) {
  ...
  return <section className="text-layers"><div className="section-heading"><h3>Text overlays</h3><span>{layers.length}/12</span></div><p className="hint">Add titles and labels independently of captions. Coordinates are percentages; end 0 means scene end. Choose each title’s Animation here, or apply a Style (animations marked ✦ animate letter by letter; Arabic animates word by word). Preview shows all layers; Render scene plays the animation.</p>
