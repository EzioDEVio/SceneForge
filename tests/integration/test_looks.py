...
# Real-world .cube files: BOM, Latin-1 titles, Resolve input range.
bom=('\ufeff'+swap).encode('utf-8')
check('LUT with a UTF-8 byte-order mark imports',client.post('/api/assets/lut',params={'project_id':pid},files={'file':('bom.cube',bom)}).status_code==200)
latin=swap.replace('TITLE "test"','TITLE "Café look"').encode('latin-1')
check('LUT with a Latin-1 title imports',client.post('/api/assets/lut',params={'project_id':pid},files={'file':('cafe.cube',latin)}).status_code==200)
...
px=np.asarray(Image.open(io.BytesIO(r.content)).convert('RGB')).reshape(-1,3).mean(axis=0)
check('graded preview shows the LUT (red and blue swapped)',r.status_code==200 and abs(px[0]-plain[2])<10 and abs(px[2]-plain[0])<10)
client.patch(f"/api/scenes/{scene['id']}",json={'look':{'lut':None}})
px2=np.asarray(Image.open(io.BytesIO(client.get(f"/api/scenes/{scene['id']}/graded-frame",params={'w':320}).content)).convert('RGB')).reshape(-1,3).mean(axis=0)
check('without a grade the preview is the plain frame',abs(px2[0]-plain[0])<10 and abs(px2[2]-plain[2])<10)
empty=client.post(f'/api/projects/{pid}/scenes',json={'title':'Empty'}).json()
check('graded preview of a scene without media is a clear 404',client.get(f"/api/scenes/{empty['id']}/graded-frame").status_code==404)
