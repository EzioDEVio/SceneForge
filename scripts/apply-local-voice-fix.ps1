# Combined Beta includes frontend, renderer and sound asset changes; project data is untouched.
param([string]$AppPath)
$ErrorActionPreference = 'Stop'
$listener = Get-NetTCPConnection -LocalPort 8000 -State Listen -ErrorAction SilentlyContinue
if ($listener) { throw "Stop the server listening on port 8000 (PID $($listener.OwningProcess -join ', ')) before updating. Use Ctrl+C in its original window." }
...
$items = @('frontend\dist', 'frontend\src', 'frontend\public', 'frontend\index.html', 'backend\app', 'assets\sfx', 'services', 'START_HERE_COMBINED.md', 'frontend\package.json', 'frontend\package-lock.json', 'scripts\start.ps1', 'scripts\local-voice.ps1', 'START_KOKORO_VOICE.bat', 'START_CHATTERBOX_VOICE.bat', 'START_HERE_LOCAL_VOICE_FIX.md')
...
$backup = Join-Path $targetRoot "_local_voice_fix_backups\$stamp"
...
Write-Host 'Local Voice Fix applied.' -ForegroundColor Green
